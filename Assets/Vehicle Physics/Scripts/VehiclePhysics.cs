...
//		GetComponentInChildren<WheelCollider>().ConfigureVehicleSubsteps(10f, 1, 1);
        ...
        //KillEngine();
        smokeObj = (GameObject) Instantiate(engineSmoke.gameObject, smokePosition.position, Quaternion.identity,
            smokePosition);
    ...
    void FixedUpdate()
    {
        if (rigid.velocity.magnitude < .01f && Mathf.Abs(_steerInput) < .01f && Mathf.Abs(_gasInput) < .01f
            && Mathf.Abs(rigid.angularVelocity.magnitude) < .01f)
            isSleeping = true;
        else
            isSleeping = false;
        ...
        if (behaviorType == BehaviorType.SportsMuscle)
        {
            if (RearLeftWheelCollider.wheelCollider.isGrounded)
                rigid.AddRelativeTorque(Vector3.up * (((steerInput * _gasInput) * direction)) / 1f,
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        if (useDamage)
        {
            if (hitTime == 0 && col.relativeVelocity.sqrMagnitude * damageFactor > 1 && strength < 1)
            {
                ...
                {
                    if (transform.InverseTransformPoint(curCol.point).y > collisionIgnoreHeight
                        && (1 << col.gameObject.layer) != 0)
                    {
                        colsChecked++;
                        ...
                        if (crashClips.Length > 0)
                        {
                            ...
                            {
                                crashSound = CreateAudioSource.NewAudioSource(gameObject, "Crash Sound AudioSource", 5,
                                    20, commonSettings.maxCrashSoundVolume,
            ...
            switch (steeringWheelRotateAround)
            {
                case SteeringWheelRotateAround.XAxis:
                    SteeringWheel.transform.localRotation =
                        orgSteeringWheelRot * Quaternion.AngleAxis(
                            ((FrontLeftWheelCollider.wheelCollider.steerAngle) * steeringWheelAngleMultiplier),
                            Vector3.right);
                    break;
    ...
    {
        engineRunning = false;
        engineStartSound = CreateAudioSource.NewAudioSource(gameObject, "Engine Start AudioSource", 5, 10, 1,
    ...
    void Engine()
    {
        ...
        float wheelRPM = _wheelTypeChoise == WheelType.FWD
            ? (FrontLeftWheelCollider.wheelRPMToSpeed + FrontRightWheelCollider.wheelRPMToSpeed)
            : (RearLeftWheelCollider.wheelRPMToSpeed + RearRightWheelCollider.wheelRPMToSpeed);

        rawEngineRPM = Mathf.Clamp(Mathf.MoveTowards(rawEngineRPM, (maxEngineRPM * 1.1f) *
                                                                   (Mathf.Clamp01(
                                                                       Mathf.Lerp(0f, 1f, (1f - clutchInput) *
                                                                                          (((wheelRPM * direction) / 2f)
                                                                                           / maxSpeedForGear[
                                                                                               currentGear])) +
        ...
        engineRPM = Mathf.Lerp(engineRPM, rawEngineRPM,
            Mathf.Lerp(Time.fixedDeltaTime * 5f, Time.fixedDeltaTime * 50f, rawEngineRPM / maxEngineRPM));
    ...
    void SoundsInitialize()
    {
        switch (audioType)
        {
            case AudioType.OneSource:
                ...
                engineSoundHigh = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound High AudioSource", 5, 50,
                    0, engineClipHigh, true, true, false);

                if (autoCreateEngineOffSounds)
                {
                    engineSoundHighOff = CreateAudioSource.NewAudioSource(gameObject,
            ...
            case AudioType.TwoSource:
                ...
                engineSoundHigh = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound High AudioSource", 5, 50,
                    0, engineClipHigh, true, true, false);
                engineSoundLow = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound Low AudioSource", 5, 25, 0,
                    engineClipLow, true, true, false);

                if (autoCreateEngineOffSounds)
                {
                    engineSoundHighOff = CreateAudioSource.NewAudioSource(gameObject,
                        "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
                    engineSoundLowOff = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound Low Off AudioSource",
                        5, 25, 0, engineClipLow, true, true, false);
                ...
                else
                {
                    engineSoundHighOff = CreateAudioSource.NewAudioSource(gameObject,
                        "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHighOff, true, true, false);
                    engineSoundLowOff = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound Low Off AudioSource",
                        5, 25, 0, engineClipLowOff, true, true, false);
                }
            ...
            case AudioType.ThreeSource:
                ...
                engineSoundHigh = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound High AudioSource", 5, 50,
                    0, engineClipHigh, true, true, false);
                engineSoundMed = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound Medium AudioSource", 5, 50,
                    0, engineClipMed, true, true, false);
                engineSoundLow = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound Low AudioSource", 5, 25, 0,
                    engineClipLow, true, true, false);
                ...
                if (autoCreateEngineOffSounds)
                {
                    engineSoundHighOff = CreateAudioSource.NewAudioSource(gameObject,
                        "Engine Sound High Off AudioSource", 5, 50, 0, engineClipHigh, true, true, false);
                    engineSoundMedOff = CreateAudioSource.NewAudioSource(gameObject,
        ...
        engineSoundIdle = CreateAudioSource.NewAudioSource(gameObject, "Engine Sound Idle AudioSource", 5, 25, 0,
            engineClipIdle, true, true, false);
        reversingSound = CreateAudioSource.NewAudioSource(gameObject, "Reverse Sound AudioSource", 1, 10, 0,
            reversingClip, true, false, false);
        windSound = CreateAudioSource.NewAudioSource(gameObject, "Wind Sound AudioSource", 1, 10, 0, windClip, true,
            true, false);
        brakeSound = CreateAudioSource.NewAudioSource(gameObject, "Brake Sound AudioSource", 1, 10, 0, brakeClip, true,
            true, false);

        if (useNOS)
            NOSSound = CreateAudioSource.NewAudioSource(gameObject, "NOS Sound AudioSource", 5, 10, 1f, NOSClip, true,
                false, false);
        if (useNOS || useTurbo)
            blowSound = CreateAudioSource.NewAudioSource(gameObject, "NOS Blow", 1, 10, 1, null, false, false, false);
        if (useTurbo)
        {
            turboSound = CreateAudioSource.NewAudioSource(gameObject, "Turbo Sound AudioSource", .1f, .5f, 0, turboClip,
                true, true, false);
    ...
    void Sounds()
    {
        ...
        if (direction == 1)
            brakeSound.volume = Mathf.Lerp(0f, commonSettings.maxBrakeSoundVolume,
    ...
    public void EngineSounds()
    {
        ...
        float pitchLevel = Mathf.Lerp(engineSoundHigh.pitch,
            Mathf.Lerp(minEngineSoundPitch, maxEngineSoundPitch, engineRPM / 7000f), Time.fixedDeltaTime * 50f);
    ...
    void AntiRollBars()
    {
        ...
        if (groundedFL)
            travelFL = (-FrontLeftWheelCollider.transform.InverseTransformPoint(FrontWheelHit.point).y
        ...
        if (groundedFR)
            travelFR = (-FrontRightWheelCollider.transform.InverseTransformPoint(FrontWheelHit.point).y
        ...
        if (groundedRL)
            travelRL = (-RearLeftWheelCollider.transform.InverseTransformPoint(RearWheelHit.point).y
                        - RearLeftWheelCollider.wheelCollider.radius)
                       / RearLeftWheelCollider.wheelCollider.suspensionDistance;

        bool groundedRR = RearRightWheelCollider.wheelCollider.GetGroundHit(out RearWheelHit);

        if (groundedRR)
            travelRR = (-RearRightWheelCollider.transform.InverseTransformPoint(RearWheelHit.point).y
    ...
    void SteerHelper()
    {
        ...
        velocityAngle = (v.y * Mathf.Clamp(transform.InverseTransformDirection(rigid.velocity).z, -1f, 1f))
                        * Mathf.Rad2Deg;
        ...
        float angle2 = Quaternion.Angle(velocityDirection.localRotation, steeringDirection.localRotation)
                       * (normalizer);

        rigid.AddRelativeTorque(
            Vector3.up * ((angle2 * (Mathf.Clamp(transform.InverseTransformDirection(rigid.velocity).z, -10f, 10f)
                                     / 600f)) * steerHelperAngularVelStrength), ForceMode.VelocityChange);
                    ...
                    clutchInput = Mathf.Lerp(clutchInput,
                        (Mathf.Lerp(1f,
                            (Mathf.Lerp(clutchInertia, 0f,
                                ((RearLeftWheelCollider.wheelRPMToSpeed + RearRightWheelCollider.wheelRPMToSpeed) / 2f)
                                / targetSpeedForGear[0])), Mathf.Abs(_gasInput))), Time.fixedDeltaTime * 5f);
                else
    ...
    void GearBox()
    {
        ...
        else if (brakeInput < .1f && transform.InverseTransformDirection(rigid.velocity).z > -1f && direction == -1
    ...
    public IEnumerator ChangeGear(int gear)
    {
        ...
        {
            gearShiftingSound = CreateAudioSource.NewAudioSource(gameObject, "Gear Shifting AudioSource", 0f, .5f,
                commonSettings.maxGearShiftingSoundVolume,
    ...
    private void NOS()
    {
        ...
        else
        {
            ...
            if (NOSSound.isPlaying)
            {
                NOSSound.Stop();
                blowSound.clip =
                    commonSettings.blowoutClip[UnityEngine.Random.Range(0, commonSettings.blowoutClip.Length)];
                blowSound.Play();
            }
        }
    }
    ...
    private void Turbo()
    {
        ...
        turboBoost = Mathf.Lerp(turboBoost,
            Mathf.Clamp(Mathf.Pow(_gasInput, 10) * 30f + Mathf.Pow(engineRPM / maxEngineRPM, 10) * 30f, 0f, 30f),
            ...
            if (shattered)
            {
                ...
                if (!hasPivotIssues)
                {
                    if (Vector3.Distance(curDamageMesh.transform.position, damagePoint)
                        ...
                        && colMag * surfaceDot * 1 / massFactor > shattered.breakForce)
                    {
                        shattered.Shatter();
                    }
                }
                ...
                {
                    if (Vector3.Distance(curDamageMesh.transform.parent.position, damagePoint)
                        < colMag * surfaceDot * 0.1f * massFactor
                        && colMag * surfaceDot * 1 / massFactor > shattered.breakForce)
                    {
                        shattered.Shatter();
    ...
    void Smoke()
    {
        if (smokeObj)
        {
            ...
            p.rateOverTime =
                new ParticleSystem.MinMaxCurve(health < 70 ? initialSmokeEmission * (1 - health / 100) : 0);
        }
    }
    ...
    void OnDrawGizmos()
    {
        ...
        if (Application.isPlaying)
        {
            ...
            for (int i = 0; i < allWheelColliders.Length; i++)
            {
                ...
                Gizmos.matrix = Matrix4x4.TRS(allWheelColliders[i].transform.position,
                    Quaternion.AngleAxis(-90, Vector3.right), Vector3.one);
    ...
    public void TorqueCurves()
    {
        ...
        if (autoGenerateTargetSpeedsForChangingGear)
        {
            for (int k = 0; k < totalGears - 1; k++)
                targetSpeedForGear[k] = Mathf.Lerp(0, maxspeed * Mathf.Lerp(0f, 1f, gearShiftingThreshold),
                    ((float) (k + 1) / (float) (totalGears)));
        }
        ...
        if (autoGenerateGearCurves)
        {
            if (orgMaxSpeed != maxspeed || orgGearShiftingThreshold != gearShiftingThreshold)
            {
                ...
                {
                    Debug.LogError(
                        "You are trying to set your vehicle gear to 0 or below! Why you trying to do this???");
                    totalGears = 1;
                    return;
                }
                ...
                for (int i = 0; i < totalGears; i++)
                {
                    if (i != 0)
                    {
                        engineTorqueCurve[i].MoveKey(0,
                            new Keyframe(0, Mathf.Lerp(1f, .05f, (float) (i + 1) / (float) totalGears)));
                        engineTorqueCurve[i].AddKey(Mathf.Lerp(0, maxspeed * .5f, ((float) (i) / (float) (totalGears))),
                            Mathf.Lerp(1f, .5f, ((float) (i) / (float) (totalGears))));
                        engineTorqueCurve[i]
                            .AddKey(Mathf.Lerp(0, maxspeed * 1f, ((float) (i + 1) / (float) (totalGears))), .15f);
                        engineTorqueCurve[i]
                            .AddKey(Mathf.Lerp(0, maxspeed, ((float) (i + 1) / (float) (totalGears))) * 2f, -3f);
    ...
    public void CreateWheelColliders()
    {
        ...
        {
            Debug.LogError(
                "You haven't choose your Wheel Models. Please select all of your Wheel Models before creating Wheel Colliders. Script needs to know their sizes and positions, aye?");
            return;
        }
