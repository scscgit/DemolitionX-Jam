...
public class VehiclePhysicsWheelCollider : MonoBehaviour
{
    ...
    private List<VehiclePhysicsWheelCollider>
        allWheelColliders =
            new List<VehiclePhysicsWheelCollider>(); // All other wheelcolliders attached to this vehicle.
...
//		allWheelColliders.Remove(this);
        ...
        switch (carController.behaviorType)
        {
            ...
            case VehiclePhysics.BehaviorType.SportsMuscle:
                ...
                {
                    Debug.LogError("Current behavior mode is ''SportsMuscle'', but your vehicle named "
                                   + carController.name
                                   + " was FWD. You have to use RWD, AWD, or BIASED to rear wheels. Setting it to *RWD* now. ");
        ...
        if (commonSettings.useSharedAudioSources)
        {
            if (!carController.transform.Find("All Audio Sources/Skid Sound AudioSource"))
                audioSource = CreateAudioSource.NewAudioSource(carController.gameObject, "Skid Sound AudioSource", 5f,
                    50f, 0f, audioClip, true, true, false);
            else
                audioSource = carController.transform.Find("All Audio Sources/Skid Sound AudioSource")
                    .GetComponent<AudioSource>();
        }
        else
        {
            audioSource = CreateAudioSource.NewAudioSource(carController.gameObject, "Skid Sound AudioSource", 5f, 50f,
        ...
        if (!commonSettings.dontUseAnyParticleEffects)
        {
            ...
            {
                GameObject ps = (GameObject) Instantiate(groundMaterials.frictions[i].groundParticles,
    ...
    void FixedUpdate()
    {
        ...
        if (carController.ExtraRearWheelsCollider.Length > 0
            && carController.applyEngineTorqueToExtraRearWheelColliders)
        {
    ...
    public void WheelAlign()
    {
        ...
        if (grounded)
            newCompression =
                1f - ((Vector3.Dot(transform.position - GroundHit.point, transform.up)
        ...
        wheelModel.rotation = transform.rotation
                              * Quaternion.Euler(wheelRotation, wheelCollider.steerAngle, transform.rotation.z);
        ...
        float extension =
            (-wheelCollider.transform.InverseTransformPoint(GroundHit.point).y
             - (wheelCollider.radius * transform.lossyScale.y)) / wheelCollider.suspensionDistance;
        Debug.DrawLine(GroundHit.point, GroundHit.point + transform.up * (GroundHit.force / rigid.mass),
    ...
    void SkidMarks()
    {
        ...
        totalSlip = Mathf.Lerp(totalSlip, (wheelSlipAmountSideways + wheelSlipAmountForward), Time.fixedDeltaTime * 3f)
                    / 1f;
    ...
    void ApplyMotorTorque(float torque)
    {
        if (carController.TCS)
        {
            ...
            if (Mathf.Abs(wheelCollider.rpm) >= 100)
            {
                if (hit.forwardSlip > startSlipValue)
                {
                    ...
                    torque -= Mathf.Clamp(torque * (hit.forwardSlip) * carController.TCSStrength, 0f,
                        carController.engineTorque);
                }
                else
                {
                    carController.TCSAct = false;
                    torque += Mathf.Clamp(torque * (hit.forwardSlip) * carController.TCSStrength,
                        -carController.engineTorque, 0f);
                }
            }
            else
            {
        ...
        wheelCollider.motorTorque =
            ((torque * (1 - carController.clutchInput) * carController._boostInput) * carController._gasInput)
            * (carController.engineTorqueCurve[carController.currentGear]
                .Evaluate(wheelRPMToSpeed * carController.direction) * carController.direction);
    }

    public void ApplySteering()
    {
        wheelCollider.steerAngle = Mathf.Clamp(carController.steerAngle * carController._steerInput,
            -carController.steerAngle, carController.steerAngle);
    }
    ...
    int GetGroundMaterialIndex()
    {
        ...
        if (!contacted && physicsMaterials.useTerrainSplatMapForGroundFrictions
                       && GroundHit.collider.sharedMaterial == groundMaterials.terrainPhysicMaterial)
        {
