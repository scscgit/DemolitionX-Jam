...
public class Exhaust : MonoBehaviour {
	...
	void Start () {
		...
		if(flame){
			...
			flameSource = CreateAudioSource.NewAudioSource(gameObject, "Exhaust Flame AudioSource", 10f, 25f, 1f, CommonSettings.exhaustFlameClips[0], false, false, false);
			flameLight.renderMode = CommonSettings.useLightsAsVertexLights ? LightRenderMode.ForceVertex : LightRenderMode.ForcePixel;
		...
		if(carController.engineRunning){
			...
			if(((carController.useExhaustFlame && carController.engineRPM >= 5000 && carController.engineRPM <= 5500 && carController._gasInput <= .25f && flameTime <= .5f) || carController._boostInput >= 1.5f) || previewFlames){
				...
				if(!flameSource.isPlaying){
					flameSource.clip = CommonSettings.exhaustFlameClips[Random.Range(0, CommonSettings.exhaustFlameClips.Length)];
					flameSource.Play();
				}
